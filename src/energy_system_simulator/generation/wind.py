...
class WindFarm:
    ...
    def output_mw(self, wind_speed_m_s: npt.ArrayLike) -> FloatArray:
        ...
        partial = (speed >= self.config.cut_in_speed_m_s) & (speed < self.config.rated_speed_m_s)
        rated = (speed >= self.config.rated_speed_m_s) & (speed < self.config.cut_out_speed_m_s)

        numerator = speed[partial] ** 3 - self.config.cut_in_speed_m_s**3
        denominator = self.config.rated_speed_m_s**3 - self.config.cut_in_speed_m_s**3
